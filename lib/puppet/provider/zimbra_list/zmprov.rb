...
## AS comments start with '##'
    ...
    def self.instances
        ...
        raw_lists.compact.map  { |i| 
            # getting uid
            ## AS We need to split the text into an array
            name = i.split("\n").grep(/uid: /)[0].gsub('uid: ','').chomp
