...
class puppet_infrastructure::firewall_addon_openvpn_server (
    ...
    firewall { '201 accept openvpn': proto => 'udp', dport => $port, action => 'accept' }
    ...
    firewall { '203 allow flow from exterior into br0': chain => 'FORWARD', iniface => $wan_iface, outiface => $lan_bridge_iface, proto => 'all', state => ['RELATED', 'ESTABLISHED'], action => 'accept' }
    ...
    # this is not directly related to firewall but
    # might be a good place for activating
